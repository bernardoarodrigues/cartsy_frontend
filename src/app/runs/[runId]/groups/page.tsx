      ...
      <div className="p-6 space-y-4">
        <Card>
          <CardContent className="p-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div className="md:col-span-3">
              <Label>Min offers: {minOffers}</Label>
              <Slider className="mt-3" min={1} max={25} step={1} value={[minOffers]} onValueChange={(v) => { setMinOffers(Array.isArray(v) ? v[0] : (v as number)); setOffset(0); }} />
            </div>
            <div className="md:col-span-3">
              <Label>Min confidence: {minConfidence.toFixed(2)}</Label>
              <Slider className="mt-3" min={0} max={1} step={0.01} value={[minConfidence]} onValueChange={(v) => { setMinConfidence(Array.isArray(v) ? v[0] : (v as number)); setOffset(0); }} />
            </div>
        ...
        <Card>
          <Table>
            ...
            <TableBody>
              {isFetching && !data
                ...
                : data?.groups.map((g) => (
                    <TableRow key={g.dedupe_id} className="hover:bg-muted/40">
                      ...
                      <TableCell className="text-right"><Badge variant="secondary">{g.num_offers}</Badge></TableCell>
                      <TableCell className="text-right tabular-nums">{Number(g.cluster_confidence).toFixed(2)}</TableCell>
                      <TableCell>
          ...
          <div className="flex items-center justify-between p-3 border-t text-sm">
            <div className="text-muted-foreground">{formatNumber(total)} groups · page {page} of {pages}</div>
