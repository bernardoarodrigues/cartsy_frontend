        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card>
                <CardContent className="p-5 space-y-1">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium">Test pairs</div>
                  <div className="text-2xl font-semibold tabular-nums tracking-tight leading-none">{formatNumber(m?.test_pairs ?? null)}</div>
                  <div className="text-xs text-muted-foreground pt-0.5"><span className="tabular-nums">{formatNumber(m?.positive_pairs ?? null)}</span> pos · <span className="tabular-nums">{formatNumber(m?.negative_pairs !== undefined && m?.positive_pairs !== undefined ? m.negative_pairs : null)}</span> neg</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5 space-y-1">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium">False positives</div>
                  <div className="text-2xl font-semibold tabular-nums tracking-tight leading-none">{formatNumber(data.false_positive_count)}</div>
                  <div className="text-xs text-muted-foreground pt-0.5">at current threshold</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5 space-y-1">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium">False negatives</div>
                  <div className="text-2xl font-semibold tabular-nums tracking-tight leading-none">{formatNumber(data.false_negative_count)}</div>
                  <div className="text-xs text-muted-foreground pt-0.5">at current threshold</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5 space-y-1">
                  <div className="text-[11px] text-muted-foreground uppercase tracking-wider font-medium">Features</div>
                  <div className="text-2xl font-semibold tabular-nums tracking-tight leading-none">{m?.feature_columns?.length ?? "—"}</div>
            ...
            <div className="grid lg:grid-cols-2 gap-6 items-stretch">
              <Card>
                <CardHeader><CardTitle className="text-sm font-semibold">Feature coefficients</CardTitle></CardHeader>
                <CardContent>
                  <ChartContainer
              ...
              <div className="flex flex-col gap-6">
                <Card>
                  <CardHeader><CardTitle className="text-sm font-semibold">Threshold curve</CardTitle></CardHeader>
                  <CardContent>
                    <ChartContainer
                      config={{
                ...
                <Card className="flex-1">
                  <CardHeader><CardTitle className="text-sm font-semibold">Feature contract ({m?.feature_columns?.length ?? 0} features)</CardTitle></CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-3 gap-1.5">
                      {(m?.feature_columns ?? []).map((f) => (
                        <Badge key={f} variant="secondary" className="font-mono text-xs justify-center truncate">{f}</Badge>
                      ))}
                    </div>
                  </CardContent>
                </Card>
            ...
            {data.top_risky_clusters.length > 0 && (
              <Card>
                <CardHeader><CardTitle className="text-sm font-semibold">Top risky predicted clusters</CardTitle></CardHeader>
                <CardContent className="px-0">
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent">
                        {Object.keys(data.top_risky_clusters[0]).map((h) => (
                          <TableHead key={h} className="first:pl-4 last:pr-4">{h}</TableHead>
                        ))}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {data.top_risky_clusters.map((row, i) => (
                        <TableRow key={i} className="hover:bg-muted/50">
                          {Object.values(row).map((v, j) => (
                            <TableCell key={j} className="text-xs font-mono first:pl-4 last:pr-4">{v}</TableCell>
                          ))}
                        </TableRow>
                      ))}
                    </TableBody>
